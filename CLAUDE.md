...
> ## 🚦 CURRENT STATE (2026-07-20) — read this, not the stale paragraph below
> **CH-00 → CH-19 are code-complete. CH-19 (Acceptance — the LAST chunk) is BUILT and MERGED to `main`
> (`6a517e5`, no-ff, pushed).** `pnpm replay` drives all six `docs/product-picture.md` scenarios through
> the real in-process pipeline; `test/acceptance/replay.test.ts` keeps them in `pnpm check`.
> **The ONLY outstanding item is `v1.0.0`, and it is PAUL'S GATE:** the DoD needs his LIVE human pass
> (play the six on the test line vs real Claude, review voice vs `kb/source/voice-guide.md`, record the
> sign-off line in `progress.md`), then tag. No `vCH-19`/`v1.0.0` tag exists yet — deliberate.
> **`progress.md` is authoritative** for what each chunk shipped. The "Current state" paragraph that
> follows is STALE (it reads CH-12 / "next CH-13") and is kept only for its still-valid eZee/Meta traps.
