...
> replay` drives all six `docs/product-picture.md` scenarios through the real in-process pipeline;
> `test/acceptance/replay.test.ts` keeps them green in `pnpm check` (full suite **1777**; CI green on the
> code commit `19ce832`).
> **Paul signed off the LIVE voice pass 2026-07-21** — S1 on the test line vs REAL Claude returned an
> accurate live ₹ quote + a graceful same-type alternative, the verbatim no-discount phrasebook line, and
...
> **`progress.md` is authoritative** for what each chunk shipped. The "Current state" paragraph that
> follows is STALE (it reads CH-12 / "next CH-13" — there is NO next chunk) and is kept ONLY for its
> still-valid eZee/Meta/OQ-19 traps.
...
- `pnpm replay` — **CH-19 acceptance**: replays all six `docs/product-picture.md` scenarios through the real in-process pipeline (scripted Claude + fixture website/eZee + captured WA sends), prints PASS/FAIL, exits non-zero on failure. Needs only `docker compose up -d postgres` (it provisions the test DB itself). The same assertions run inside `pnpm check` via `test/acceptance/replay.test.ts`. **Never run two vitest/replay processes at once** — they share `nistula_test` and TRUNCATE each other, faking failures.
...
- Deploy: merging to `main` auto-deploys behind the `/health` gate. Pre-merge live demo = `railway up` from the chunk branch. **⚠️ NOTE (2026-07-21): auto-deploy from `main` appears DISCONNECTED — the prod box was still on CH-18c after the CH-19 merge (uptime never reset). VERIFY Railway actually picked up the target commit (compare `/health` uptime / the deployment commit) before assuming a merge shipped; reconnect via `railway service source connect` if needed (see progress.md CH-19 clean-ups).**
