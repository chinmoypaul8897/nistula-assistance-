...
**Current state: CH-00 → CH-07 merged, tagged and LIVE on Railway; CH-08 (Short-term memory) BUILT + audited on `chunk/CH-08-short-term-memory`, merging via PR #24 (light live probe pending — flip this line to DONE post-merge).** The service takes real WhatsApp messages on the Meta test number, replies in Nistula's voice through Claude, quotes live website-identical prices via tools, answers villa/policy/FAQ questions from the compiled knowledge base (prompt block [3]), brackets every model turn with deterministic code (§6.7 policy routing before, the complete §6.5 guardrail pipeline after — every hit persisted to `raw_events`), and now has §6.3 short-term memory: a token-budgeted transcript window (≤30 msgs/~6k tokens), a rolling per-conversation summary rendered as the `[EARLIER CONTEXT]` block (untrusted DATA, non-evidence), and a nightly 04:00 IST summariser plus an on-demand overflow path (runbook §CH-08). **Next chunk: CH-09 (Long-term memory) — its recorded forward pointers: the fuller profile/fact injection red-team pack, and block [5] grows facts/stays via `buildGuestBlock`.** `progress.md` is authoritative for exactly what exists and what each chunk learned — read it, not this paragraph, for detail.
