...
async function ctxFor(
  ...
  const stays = projectAll(await getGuestStays(db, guest.id));
...
describe('the reference claim — every failure is the SAME value', () => {
  ...
  // THE assertion. Six different failures, one indistinguishable value.
  it('returns a byte-identical refusal for every failure path', async () => {
    ...
    // 6. the model invented a reference the guest never typed
    results.push(
      JSON.stringify(await run({ reference: '953' }, await ctxFor(STRANGER, 'hello there'))),
    );

    // ONE value across all six. This is the security property.
  ...
  it('escalates a refusal to a human, and records the strike', async () => {
    await upsertMirrorRow(db, makeInput({ guestPhone: null }));
  ...
  // a guardrail regenerate (CH-09's `saves` counter solved the same shape).
  it('charges one strike per turn even when the loop re-runs', async () => {
    await upsertMirrorRow(db, makeInput({ guestPhone: null }));
    const ctx = await ctxFor(STRANGER, 'booking 953, Priya Sharma, 26 Aug');

    await run({ reference: '953' }, ctx); // first loop
  ...
  // person must tell them, not the AI.
  it('links but refuses to describe a verified claim on a cancelled booking', async () => {
    ...
    expect(ctx.booking?.claim.escalate).toBe(true);
    const guest = await upsertGuestByPhone(db, STRANGER, 'x');
    expect(await getGuestStays(db, guest.id)).toHaveLength(1); // it IS theirs
  });
});
