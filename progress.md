...
| CH-18a-1 | Security hardening + guest erasure | ✅ DONE 2026-07-19 — `chunk/CH-18a-1-security-erasure` (**1720** tests; DELETE_GUEST one-tx anonymise-in-place + dry-run + residue-sweep contract test, `POST /admin/delete-guest`, `maybeRegisterAdminRoutes` 404-when-disabled test, standalone secrets-shaped redaction fixture, rate-limit/cool-off confirmed final, audit clean at CI gate. Two send/lifecycle TODOs deferred → CH-18c) | [↓](#ch-18a-1--security-hardening--guest-erasure-delete_guest--done-2026-07-19) |
| CH-18a-2 | Backups + keep-alive + runbook + go-live | ⬜ pending (off-site R2/B2 + `age` chosen) | |
...
**How to verify:**
- `pnpm check` (exit code) — 1720 green incl. `test/erasure.test.ts` (cascade + residue sweep +
  dry-run + idempotency + `redactPayload`) and the delete-guest / 404-when-disabled cases in
  `test/admin-route.test.ts` and the redaction fixture in `test/logger.test.ts`.
