...
- **Current chunk pointer:** CH-04 (Brain v1 — voice) — **BUILT; PR #9 open with CI green (Node 22 + 24); NOT yet merged.** Remaining before it lands: Paul's live 10-message phone demo (Definition of done), then merge to `main` + tag `vCH-04`. `pnpm check` green at 188 tests; a live Anthropic integration smoke passed (voice on-target, price deferred with no invented ₹, static prompt head caches — 1655 tokens written on msg 1, read back on msg 2). The service now REPLIES in Nistula's voice instead of echoing; no tools yet, so every factual/price/availability question is deferred (correct until CH-05). **`ANTHROPIC_API_KEY` is required at boot from CH-04** (set in local `.env` + Railway, live-validated). Next chunk after CH-04 merges: CH-05 (Price tools). CH-00/CH-00b/CH-01/CH-02/CH-03 merged and tagged (`vCH-00`…`vCH-03`).
...
| CH-04 | Brain v1 — voice | 🟨 BUILT — PR #9, demo+merge pending | [↓](#ch-04--brain-v1--the-voice--built-2026-07-11) |
...
### CH-04 · Brain v1 — the voice — BUILT 2026-07-11

*(Built + `pnpm check` green (188) + live Anthropic integration smoke passed; PR #9 open with CI green on Node 22 + 24. NOT yet merged — remaining: Paul's 10-message phone demo (Definition of done), then merge to `main` + tag `vCH-04`.)*
