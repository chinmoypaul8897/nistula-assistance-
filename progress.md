...
| CH-19 | Acceptance — six scenarios | ✅ **DONE 2026-07-21 — `v1.0.0` TAGGED** (Paul signed off the live voice pass). `pnpm replay` 6/6, full suite green at 1777; TWO adversarial review rounds RED→fixed (round 2 caught a false-green in round 1's own status-gate fix). **PROJECT COMPLETE (CH-00 → CH-19).** | [↓](#ch-19--acceptance--the-six-scenarios--done-2026-07-21--v100-tagged) |
...
### CH-19 · Acceptance — the six scenarios — DONE 2026-07-21 · v1.0.0 TAGGED
...
- `takeoverDeps.now` → `nowIST()` (in `jobs/index.ts`), so `human_active_until` is written and read on
  ONE clock. **(A third leg — the staff-command worker's `now` → `nowIST()` — was added here mid-build
  then REVERTED in review round 2 as unjustified; it stays `new Date()`. So the SHIPPED prod surface is
  exactly TWO: `repos.ts` dbNow + `jobs/index.ts` takeoverDeps.now.)** **Both shipped changes are
  byte-identical in production** (`FAKE_NOW_IST` is boot-REFUSED there, §3.7) — the prod-change reviewer
  verified the guard fires at boot and the field is in the zod schema (so the guard actually runs); no
  regression across the full 1777-test suite.
...
- **REVERTED round 1's staff-command clock change** (`jobs/index.ts`): prod-inert, test-inert, its
  ...
  agreed. **CH-19's shipped production surface is exactly TWO justified changes: `repos.ts` turn `dbNow`
  + `jobs/index.ts` `takeoverDeps.now → nowIST()`** (only the staff-command leg was reverted).
...
**🚦 POST-v1.0.0 FAST-FOLLOW (Paul-approved 2026-07-21, option b) — the ONE unbuilt product-picture
beat:** `docs/product-picture.md` S2 specified *"STAFF: arrivals digest line the evening before"* and it
was **never built in any chunk** (the round-2 completeness critic caught it). The lifecycle ships a
morning 10:00 staff digest (`staff/digest.ts`) + a 23:30 ops rollup, but no evening "arriving tomorrow"
line. **Decision: amend the contract, not build it under v1.0.0** — a staff-convenience beat, not a
guest-facing gap (the pre-arrival guest message + morning digest cover the core). The product-picture S2
line is struck in place. **Build spec for the fast-follow:** a ~17:00-IST cron mirroring
`runMorningDigest` — query `bookings_mirror` for tomorrow's arrivals (gated like the lifecycle: epoch/
status/date/source + phone), send OPS a one-line "arriving tomorrow: N" summary via the window-aware
`nst_digest` template; fail-quiet on an empty day. Small, self-contained, no migration.
...
**✅ PAUL'S ACCEPTANCE SIGN-OFF — 2026-07-21. `v1.0.0` TAGGED.** The live human pass ran on the test
line against REAL Claude (S1 fully runnable now). Three live guest turns, all returning verbatim/in
Nistula's voice, all confirmed accurate by Paul:
1. *"is a 3bhk villa free 20-22 dec and whats the rate"* → **"The 3bhk villas are taken for those dates.
   The apartments are free — ₹59,000 for 20–22 Dec, all three units available. Would that work?"** — a
   LIVE, accurate price (Paul confirmed the villas genuinely are taken), a graceful same-type
   alternative, British English, no exclamation, ends on a soft question.
2. *"any discount for direct booking?"* → the `PHRASEBOOK.discountAsk` line **verbatim** (no discount,
   no apology, no number — the negotiation lock held live).
3. *"are you a bot?"* → the sanctioned identity line **verbatim**.
**Paul: "yes" — reads as Nistula.** Voice invariant #6 (the one a scripted harness cannot prove) is
confirmed LIVE. **The unblock that made this possible:** Paul's test number (+91 99038 76744) was
polluted by three leftover test bookings (957/970 cancelled + **969** the hand-corrupted one), which
made every reply route to `booking_undescribable` → a forced draft → dead with `OPS_NUMBERS` empty →
silence. Cleared in one prod tx (969 marked cancelled + its 4 armed lifecycle rows disarmed; all three
phones nulled so they never re-link; the guest's 3 stays unlinked) → the number became a clean lead →
replies send directly. **The cutover live-replay legs stay deferred (Q1):** S3/S4 need a 2nd allowlisted
number + roster; S2/S6 need approved templates — runbook §CH-19.

**Acceptance report:** all six product-picture scenarios pass deterministically in `pnpm replay` /
`pnpm check` (full suite green at 1777); the live voice pass confirms the model voice the harness
stubs; the product-picture contract is honest (the one unbuilt beat — the evening arrivals digest — is
struck + filed as a post-v1 fast-follow, above). **CH-19 is DONE; `v1.0.0` tagged and pushed.**

**Two clean-ups tracked separately (NOT v1.0.0 blockers):** (1) `WEBSITE_BASE_URL` villa-map drift —
the *apartment* IDs quote fine live (₹59,000 proved it), but the *villa* (3bhk) IDs returned
`villa_map_drift` 404s; the answer happened to be right (villas taken), but on villa-available dates the
